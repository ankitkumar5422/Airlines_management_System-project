//made By Ankit kumar @ankitkumar5422 form bihar
#include <string.h>
#include <stdlib.h>
#include <stdio.h>
#include <ctype.h>
...
{				
	...
	public:
		/* --ADD/REMOVE FLIGHTS--*/
		void addFlight(); //d?a?e????eta? t?? p??s???? pt?se?? (ep????? µe??? ?? 1)
		static void deleteFlight(int num); //d?a?e????eta? t?? afa??es? pt?se?? (ep????? µe??? ?? 2)
		
		/* --DISPLAY FLIGHTS--*/
		static void displayFlightInfo(int num); //p??ß???e? t?? p????f???e? µ?a? pt?s?? (ep????? µe??? ?? 3)
		static void displaySchedule(); //p??ß???e? t?? pt?se?? p?? ?p?????? ?ata????µ??e? st? s?st?µa (ep????? µe??? ?? 4)
		
		/* --BOOK SEATS--*/
		static void resSeat(int num, int val); //a???µe???e? t?? ?at?????µ??e? ??se?? µ?a? pt?se??
...
class Queue // class that stores passengers on each flight's queue
{
	...
	public:
		...
		/* --??????? S??????O? ????S-- */
		static void display(int num); //eµfa???e? t??? pe??te?, p?? ß??s???ta? ?at?????µ???? 
									 //st?? ???? a?aµ???? t?? e??st?te pt?s??

		/* --???S???F? ??????????? ???S??-- */
		Person oldest(); //ep?st??fe? t? a?t????µe?? t?? a??a??te??? pe??t?, µ?s? t?? ???s?? Person
